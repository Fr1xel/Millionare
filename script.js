...
let randomGame = game[player.tezina].sort(() => Math.random() - 0.5);

const button1 = document.querySelector("#box1");
const tezinaHolder = document.querySelector("#tezinaHolder")
...
function startQuestions() {
  player.timeQuestion = 0;
  randomGame[player.currentIndex].answers = game[player.tezina][player.currentIndex].answers.sort(() => Math.random() - 0.5);
...
function setQuestions(butn, index){
  butn.innerText = game[player.tezina][player.currentIndex].answers[index]
}
startQuestions();
...
function buttonClick(event) {
  if (
    ...
    if (player.currentIndex === game.lagano + game.srednje + game.tesko - 1) {
      highScoreSetter();
...
function settingDifficulty(){
  if(player.points === game.lagano.length){
    player.currentIndex = 0
